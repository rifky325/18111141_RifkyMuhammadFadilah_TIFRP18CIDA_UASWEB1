...
if(isset($_POST['login'])) { // mengecek apakah form pada login.php variabelnya ada isinya
    ...
    try {
        $sql = "SELECT * FROM `register` WHERE username = ? AND password = SHA1(?)"; // buat queri select
        ...
        if($count == 1) { // jika rownya ada 
            ...
            header("Location: ../halamansesudahlogin.php"); // lempar variabel ke tampilan index.php
            return;
        }else{
